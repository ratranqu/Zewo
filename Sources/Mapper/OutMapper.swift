...
fileprivate extension OutMapperProtocol {
    ...
    mutating func set(_ map: Destination, at indexPath: [Key]) throws {
        do {
...
extension OutMapperProtocol {
    ...
    public mutating func unsafe_map<T>(_ value: T, to indexPath: Key...) throws {
    ...
    ///
    /// - throws: `OutMapperError`.
    public mutating func map<T : OutMappable>(_ value: T, to indexPath: Key...) throws {
    ...
    ///
    /// - throws: `OutMapperError`.
    public mutating func map<T : BasicOutMappable>(_ value: T, to indexPath: Key...) throws {
        let new: Destination = try value.map()
        try set(new, at: indexPath)
    }
    ...
    ///
    /// - throws: `OutMapperError`.
    public mutating func map<T : OutMappableWithContext>(_ value: T, to indexPath: Key..., withContext context: T.MappingContext) throws {
        let new = try value.map(context: context) as Destination
        try set(new, at: indexPath)
    }
    ...
    ///
    /// - throws: `OutMapperError`.
    public mutating func unsafe_mapArray<T>(_ array: [T], to indexPath: Key...) throws {
    ...
    ///
    /// - throws: `OutMapperError`.
    public mutating func map<T : OutMappable>(_ array: [T], to indexPath: Key...) throws {
    ...
    ///
    /// - throws: `OutMapperError`.
    public mutating func map<T : BasicOutMappable>(_ array: [T], to indexPath: Key...) throws {
        let maps: [Destination] = try array.map({ try $0.map() })
        let map = try arrayMap(of: maps)
        try set(map, at: indexPath)
    }
    ...
    ///
    /// - throws: `OutMapperError`.
    public mutating func map<T : OutMappableWithContext>(_ array: [T], to indexPath: Key..., withContext context: T.MappingContext) throws {
        let maps: [Destination] = try array.map({ try $0.map(context: context) })
        let map = try arrayMap(of: maps)
        try set(map, at: indexPath)
    }
...
extension ContextualOutMapperProtocol {
    ...
    ///
    /// - throws: `OutMapperError`.
    public mutating func map<T : OutMappableWithContext>(_ value: T, to indexPath: Key...) throws where T.MappingContext == Context {
        let new: Destination = try value.map(context: self.context)
        try set(new, at: indexPath)
    }
    ...
    ///
    /// - throws: `OutMapperError`.
    public mutating func map<T : OutMappableWithContext>(_ array: [T], to indexPath: Key...) throws where T.MappingContext == Context {
        let maps: [Destination] = try array.map({ try $0.map(context: context) })
        let map = try arrayMap(of: maps)
        try set(map, at: indexPath)
    }
...
/// Object that maps strongly-typed instances to structured data instances.
public struct OutMapper<Destination : OutMap, K : MappingKey> : OutMapperProtocol {
...
public struct ContextualOutMapper<Destination : OutMap, K : MappingKey, Context> : ContextualOutMapperProtocol {
